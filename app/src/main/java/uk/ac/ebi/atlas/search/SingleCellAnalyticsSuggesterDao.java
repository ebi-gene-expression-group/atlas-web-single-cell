...
public class SingleCellAnalyticsSuggesterDao implements AnalyticsSuggesterDao {
    ...
    private Stream<Suggestion> fetchOntologySuggestions(String query, int limit, Species... species) {
        ...
        var compareByWeightLengthAlphabetical =
                Comparator.comparingLong(Suggestion::getWeight).reversed().thenComparingInt(suggestion -> suggestion.getTerm().length()).thenComparing(Suggestion::getTerm);
        ...
        var dictionaries = new String[]{"ontologyAnnotationSuggester", "ontologyAnnotationAncestorSuggester",
    ...
    private Stream<Suggestion> getSuggestionStream(int limit,
        ...
        try {
            suggestionStream =
                    singleCellAnalyticsCollectionProxy.solrClient.query("scxa-analytics", solrQuery).getSuggesterResponse().getSuggestions().values().stream().flatMap(List::stream)
                    // The current implementation considers symbols Aspm and ASPM two different suggestions. I dont’t
                    // know
    ...
    private SolrQuery buildSuggesterRequestHandler(String[] dictionaries, String query, Species[] species) {
        ...
        solrQuery.setRequestHandler("/suggest")
                ...
                .setParam("suggest.build", Boolean.TRUE).setParam("suggest.dictionary", dictionaries).setParam(
                        "suggest.q", query)
