...
curl "http://${SOLR_HOST}/solr/admin/collections?action=CREATEALIAS&name=scxa-analytics&collections=scxa-analytics-v5"
...
printf "\n"
printf "Run the following command (don’t worry if the request returns a 500 error after a while):\n"
printf "\n"
...
printf "\n"
printf "While the command above is running, monitor the size of the suggester directory size:\n"
printf "\n"
printf "docker exec -it scxa-solrcloud-1 bash -c 'watch du -sc server/solr/bioentities-v1*/data/*'\n"
printf "docker exec -it scxa-solrcloud-2 bash -c 'watch du -sc server/solr/bioentities-v1*/data/*'\n"
printf "\n"
printf "The suggester in all shards will be built when the propertySuggester directory size stabilises.\n"
printf "Run the above procedure for bioentitySuggester as well.\n"
