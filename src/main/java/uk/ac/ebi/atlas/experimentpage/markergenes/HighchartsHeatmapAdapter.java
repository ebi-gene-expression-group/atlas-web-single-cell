...
public class HighchartsHeatmapAdapter {
    ...
    private static final Function<CellTypeMarkerGene, Pair<String, String>> CELL_TYPE_MARKER_GENE_TO_ID_CLUSTER_ID_WHERE_MARKER =
            markerGene -> Pair.of(markerGene.geneId(), markerGene.clusterIdWhereMarker());
    ...
    public ImmutableList<Map<String, Object>> getCellTypeMarkerGeneHeatmapData(Collection<CellTypeMarkerGene> markerGenes) {
        ...
        var rows =
                sortedMarkerGenes.stream()
                        .map(CELL_TYPE_MARKER_GENE_TO_ID_CLUSTER_ID_WHERE_MARKER)
                        .distinct()
                        .collect(toImmutableList());
        ...
        return sortedMarkerGenes.stream()
                .map(markerGene ->
                        ImmutableMap.<String, Object>builder()
                                .put("x", markerGene.clusterId())   // Cluster IDs start at 1, Highcharts columns are 0-based
                                .put("y", rows.indexOf(CELL_TYPE_MARKER_GENE_TO_ID_CLUSTER_ID_WHERE_MARKER.apply(markerGene)))
                                .put("geneName", symbolsForGeneIds.getOrDefault(markerGene.geneId(), markerGene.geneId()))
                                .put("value", markerGene.medianExpression())
                                .put("clusterIdWhereMarker", markerGene.clusterIdWhereMarker())
                                .put("pValue", markerGene.pValue())
